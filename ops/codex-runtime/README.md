...
`codex-provider.js` starts the locally installed Codex CLI with the current desktop login session, `--disable plugins`, `project_doc_max_bytes=0`, an ephemeral thread, `approvalPolicy: "never"`, and a read-only sandbox with no network access. The provider sends only the public game view and that seat's own hand. It requests the fixed `gpt-5.6-luna` model at `max` effort, allows one JSON repair request, and never substitutes a rule-based player.
