...
function renderSeatList() {
  ...
  for (let seatIndex = 0; seatIndex < 4; seatIndex += 1) {
    ...
    kind.textContent = player?.kind === 'ai' ? `AI · ${aiSeatLabel(player.ai)}` : kindLabel(player?.kind || 'open');
    state.append(kind);
    if (player?.kind === 'ai') {
      const aiStatus = document.createElement('small');
      aiStatus.className = 'seat-meta';
      aiStatus.textContent = player.ai?.error
        ? `错误：${player.ai.error}`
        : (player.ai?.configured ? (player.ai?.protocol === 'codex' ? 'Codex · 已就绪' : '已配置') : '待配置');
  ...
  const codexReadyCount = (currentState?.players || []).filter((player) => player.kind === 'ai' && player.ai?.protocol === 'codex' && player.ai?.configured).length;
  $('lobby-readiness').textContent = ready ? (codexReadyCount ? `四席已就绪 · ${codexReadyCount} Codex` : '四席已就绪') : `${configuredCount}/4 席已就绪`;
  $('lobby-readiness').classList.toggle('ready', ready);
  $('start-button').disabled = !isHost || !ready;
  $('start-help').textContent = ready
    ? (codexReadyCount ? `${codexReadyCount} 个本机 Codex AI 已就绪，可以开局。` : '可以开局。AI 会使用各自的配置做决定。')
    : '真人需入座；AI 席位需先填写接口配置。';
}
...
function renderState(rawState) {
  ...
  $('hand-panel').hidden = !inRoom || inLobby || !Number.isInteger(state.selfSeat) || selfEliminated;
...
async function resumeRoom() {
  ...
  const ack = await sendAction('resume', { code: session.code, token: session.token });
  if (!ack.ok) {
    clearSession();
...
async function setupScene() {
  try {
    ...
    console.error(error);
    setConnection('2D 桌面加载失败 · 仍可使用房间面板', 'bad');
