...
export class LiarRoom {
  constructor(options = {}) {
    ...
    if (options.host) this.addEvent(`${this.players[0].name} 创建了房间`, 'system');
  }
  ...
  beginRound({ initial = false, starterSeat = null } = {}) {
    ...
    this.deadline = this.turnSeat == null ? null : this.now() + this.durations.turnMs;
  ...
  _advanceAfterPlay(playedBy) {
    ...
    this.turnSeat = next;
    this.deadline = this.now() + this.durations.turnMs;
    this.turnDeadline = this.deadline;
    this._touch();
    this._requestAIIfNeeded();
  }
  ...
  leave(seatIndex) {
    ...
    seat.connected = false;
    seat.socketId = null;
    this.addEvent(`${seat.name} 离开了房间，保留断线席位`, 'leave');
    return this.publicStateFor(null);
  }

  _requestAIIfNeeded() {
    if (this.phase !== 'playing' || this.turnSeat == null) return;
  ...
  async requestAIDecision(seatIndex = this.turnSeat, provider = this.aiProvider) {
    if (this.phase !== 'playing' || this.turnSeat !== seatIndex) return this.publicStateFor(null);
    const seat = this._assertSeat(seatIndex);
  ...
  async requestAITrigger(seatIndex = this.loserSeat, provider = this.aiProvider) {
    if (this.phase !== 'roulette' || seatIndex !== this.loserSeat || this.lastShot) return this.publicStateFor(null);
    const seat = this._assertSeat(seatIndex);
    if (seat.kind !== 'ai' || seat.ai.error) return this.publicStateFor(null);
    if (this.aiRequest) return this.publicStateFor(null);
    const requestId = ++this.aiRequestSeq;
