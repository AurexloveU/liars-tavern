...
// Existing names are retained unless --rename is explicitly supplied. No game or benchmark is run.
...
try {
  for (const preset of MODEL_PRESETS) {
    if (data.profiles[preset.id]?.name && !rename) continue;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const result = await client.runTurn({ model: preset.model, effort: preset.effort,
        ...
        text: `你是骗子酒馆中的 ${preset.label} 玩家。请为自己选一个常见、自然、现代的英文人名（first name），3至12个英文字母，首字母大写。像现实生活里的普通成年人会使用的名字，不要古风、奇幻、动物、酒馆称号、诗意绰号，不使用模型名。不要使用已有名字：${used.join('、') || '无'}。只回复 {"name":"你取的名字"}。`,
        outputSchema: { type: 'object', additionalProperties: false, properties: { name: { type: 'string', minLength: 3, maxLength: 12 } }, required: ['name'] },
      });
      try { name = parseAction(result.text).name; } catch { name = null; }
      if (typeof name === 'string' && /^[A-Z][a-z]{2,11}$/.test(name) && !used.includes(name)) break;
      name = null;
    }
